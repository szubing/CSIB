...
def build_table(dirname, models=None, n_envs=None, num_dim=None, latex=False, standard_error=False,
    ...
    for dataset in df["dataset"].unique():
        ...
        for model in df["model"].unique():
            ...
            min = table_test_errors.mean(axis=1).min(axis=0)
            table_avg[dataset][model] = f"{float(mean):.2f} {pm} {float(std):.2f} ({float(min):.2f})"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_peak", "--tp", type=int, default=20,
